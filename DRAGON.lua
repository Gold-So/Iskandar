...
if text == "/start" then
if not DevSoFi(msg) then
local Namebot = (database:get(bot_id..'Name:Bot') or 'اسكندر') 
local DRAGON_Msg = { 
...
send(msg.chat_id_, msg.id_,' ⌯︙ اصدار سورس اسكندر{ 3x☆}')
end
...
local Text = [[ 
[قناه سورس اسكندر ادخل وتابع الجديد](t.me/alexaandeer1)
]] 
keyboard = {}  
keyboard.inline_keyboard = { 
...
database:del(bot_id..'Srt:Bot') 
local Text = [[ 
 ⌯︙ من أحسن السورسات على التليجرام سورس اسكندر ⌯︙
...
print("it is Bot")
N = (database:get(bot_id.."Name:Bot") or "اسكندر")
...
local msg_id = msg.id_/2097152/0.5 
https.request("https://api.telegram.org/bot"..token..'/sendvideo?chat_id=' .. msg.chat_id_ .. '&video=https://t.me/aslak8/36&caption=' .. URL.escape(Text).."&reply_to_message_id="..msg_id.."&parse_mode=markdown&disable_web_page_preview=true&reply_markup="..JSON.encode(keyboard))
end
...
"سؤال ينرفزك ؟ ", 
" هل يعجبك سورس اسكندر؟؟ ", 
" اكثر ممثل تحبه ؟ ", 
"قد تخيلت شي في بالك وصار ؟ ", 
"شيء عندك اهم من الناس ؟ ", 
"تفضّل النقاش الطويل او تحب الاختصار ؟ ", 
"وش أخر شي ضيعته؟ ", 
"اي رايك في سورس اسكندر؟ ", 
"كم مره حبيت؟ ", 
...
local Text = [[  
 ⌯︙ اهلا في قائمه الالعاب المتطوره سورس اسكندر ⌯︙ 
...
t = ' ⌯︙ ملفات السورس اسكندر•━═━═『⌯︙ 𝘼𝙇𝙀𝙓𝘼𝙉𝘿𝙀𝙍 ⌯︙』═━═━•\n •━═━═『⌯︙ 𝘼𝙇𝙀𝙓𝘼𝙉𝘿𝙀𝙍 ⌯︙』═━═━• \n'
i = 0
for v in io.popen('ls File_Bot'):lines() do
if v:match(".lua$") then
i = i + 1
...
send(msg.chat_id_, msg.id_," ⌯︙ عذرا الملف لايدعم سورس اسكندر\n") 
end
return false
end
...
if text == ("مسح قائمه اسكندر") and Sudo(msg) then
database:del(bot_id..'CoSu'..msg.chat_id_)
send(msg.chat_id_, msg.id_, '\n ⌯︙ تم مسح قائمه اسكندر')
return false
end

if text == 'قائمه اسكندر' and Sudo(msg) then
...
if text == ("رفع اسكندر") and msg.reply_to_message_id_ and Sudo(msg) then
if AddChannel(msg.sender_user_id_) == false then
local textchuser = database:get(bot_id..'text:ch:user')
if textchuser then
send(msg.chat_id_, msg.id_,'['..textchuser..']')
...
if text and text:match("^رفع اسكندر@(.*)$") and Sudo(msg) then
local username = text:match("^رفع اسكندر@(.*)$")
if AddChannel(msg.sender_user_id_) == false then
local textchuser = database:get(bot_id..'text:ch:user')
if textchuser then
send(msg.chat_id_, msg.id_,'['..textchuser..']')
...
if text and text:match("^رفع اسكندر(%d+)$") and Sudo(msg) then
local userid = text:match("^رفع اسكندر(%d+)$") 
if AddChannel(msg.sender_user_id_) == false then
local textchuser = database:get(bot_id..'text:ch:user')
if textchuser then
send(msg.chat_id_, msg.id_,'['..textchuser..']')
...
if text == ("تنزيل اسكندر") and msg.reply_to_message_id_ and Sudo(msg) then
if AddChannel(msg.sender_user_id_) == false then
local textchuser = database:get(bot_id..'text:ch:user')
if textchuser then
send(msg.chat_id_, msg.id_,'['..textchuser..']')
...
if text and text:match("^تنزيل اسكندر@(.*)$") and Sudo(msg) then
local username = text:match("^تنزيل اسكندر@(.*)$")
if AddChannel(msg.sender_user_id_) == false then
local textchuser = database:get(bot_id..'text:ch:user')
if textchuser then
send(msg.chat_id_, msg.id_,'['..textchuser..']')
...
if text and text:match("^تنزيل اسكندر(%d+)$") and Sudo(msg) then
local userid = text:match("^تنزيل اسكندر(%d+)$") 
if AddChannel(msg.sender_user_id_) == false then
local textchuser = database:get(bot_id..'text:ch:user')
if textchuser then
send(msg.chat_id_, msg.id_,'['..textchuser..']')
...
if text == ""..(database:get(bot_id..'Name:Bot') or 'اسكندر').."" then  
Namebot = (database:get(bot_id..'Name:Bot') or 'اسكندر')
local Dev_Ah8 = {
'ننعم يروحي 😻💙',
'نعم يا قلب  '..Namebot..'',
'عاوز اي من '..Namebot..'',
...
end --الكود حصري سورس اسكندريعني لو بكتهن راح اعرفك انت الاخذتهن
...
-------------------------------
if text == ""..(database:get(bot_id..'Name:Bot') or 'اسكندر').." غادر" or text == 'غادر' then  
...
keyboard = {} 
keyboard.inline_keyboard = {
{{text = '  ❨𝗦𝗢𝗨𝗥𝗖𝗘❩ ', url="t.me/So_Offline"}},
{{text = 'اضف البوت لمجموعتك', url="http://t.me/"..sudos.UserName.."?startgroup=new"}},
} 
local function getpro(extra, result, success)
if result.photos_[0] then
...
local Name_Bot = (database:get(bot_id..'Name:Bot') or 'اسكندر')
